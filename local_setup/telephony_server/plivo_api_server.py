...
def populate_ngrok_tunnels():
    ...
    if response.status_code == 200:
        ...
        for tunnel in data['tunnels']:
            ...
            elif tunnel['name'] == 'callagent-app':
                bolna_url = tunnel['public_url'].replace('https:', 'wss:')

        if not telephony_url or not bolna_url:
            raise HTTPException(status_code=500, detail="Failed to get ngrok tunnel URLs")
        return telephony_url, bolna_url
    else:
        raise HTTPException(status_code=500, detail=f"Unable to fetch ngrok tunnels. Status code: {response.status_code}")
