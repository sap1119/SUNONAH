...
import copy
from .utils import format_messages, convert_to_request_log
...
def track_request(data, meta_info, source="system", direction="request", model=None, is_cached=False, run_id=None):
    """Track a request or response for analytics"""
    request_log = convert_to_request_log(data, meta_info, model, source, direction, is_cached, run_id)
    logger.info(f"Request log: {request_log}")
    return request_log
